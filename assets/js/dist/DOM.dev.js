  ...
  if (mode === 1) {
    ...
    for (var _i = 0; _i < data.length; _i++) {
      for (var j = 0; j < data[_i].length; j++) {
        ...
        $('#' + gender + 'sHeatDistance' + l + '').html('' + data[_i][j][0].distance + 'm');
        _k = 1;
        l++;
        $('.modal').modal();
      }
    ...
    for (var _i2 = 0; _i2 < data.length; _i2++) {
      ...
      for (var _j = 0; _j < data[_i2].length; _j++) {
        $('#' + gender + 'sTable_' + _l + '_content').append('<tr>' + '<form class="col s12">' + '<td class="center">' + '<div class="input-field  col offset-s2 col s7">' + '<input class="center ' + gender + 'Line" id="line' + data[_i2][_j]['ath_id'] + '" name="line' + data[_i2][_j]['ath_id'] + '" type="text" class="validate" value="' + _k2 + '">' + '<label class="active" for="line' + data[_i2][_j]['ath_id'] + '">Poste de tir</label>' + '</div>' + '</td>' + '<td class="center">' + data[_i2][_j]['cat_name'] + '</td>' + '<td class="center">' + data[_i2][_j]['first_name'] + ' ' + data[_i2][_j]['last_name'] + '</td>' + '<td class="center">' + '<div class="input-field  col offset-s2 col s7">' + '<input class="center ' + gender + 'Min" id="minutes_' + data[_i2][_j]['ath_id'] + '" name="minutes_' + data[_i2][_j]['ath_id'] + '" type="text" class="validate">' + '<label for="minutes_' + data[_i2][_j]['ath_id'] + '">Minutes</label>' + '</div>' + '<div class="input-field  col offset-s2 col s7">' + '<input class="center ' + gender + 'Sec" id="seconds_' + data[_i2][_j]['ath_id'] + '" name="seconds_' + data[_i2][_j]['ath_id'] + '" type="text" class="validate">' + '<label for="seconds_' + data[_i2][_j]['ath_id'] + '">Secondes</label>' + '</div>' + '</td>' + '<td class="center" id="lr_points_' + data[_i2][_j]['ath_id'] + '">0</td>' + '<td class="center" id="handicap_' + data[_i2][_j]['ath_id'] + '">' + data[_i2][_j]['lr_handicap'] + '</td>' + '<td class="center" id="arrivalInput_' + data[_i2][_j]['ath_id'] + '">' + '<div class="input-field col s8 offset-s3">' + '<input class="center ' + gender + 'arrival" id="arrival_' + data[_i2][_j]['ath_id'] + '" name="arrival_' + data[_i2][_j]['ath_id'] + '" type="text" class="validate">' + '<label for="arrival_' + data[_i2][_j]['ath_id'] + '">Place d\'arrivée</label>' + '</div></td>' + '<td class="center" id="actions_' + data[_i2][_j]['ath_id'] + '">' + '<div class="row">' + '<div class="col offset-s2 s4">' + '<a class="btn-floating waves-effect waves-light orange btn tooltipped modal-trigger" id="fouls_' + data[_i2][_j]['ath_id'] + '" href="#foulsModal_' + data[_i2][_j]['ath_id'] + '"data-position="right" data-tooltip="Pénalités"><i class="material-icons addFouls">flag</i></a>' + '</div>' + '<div class="col s2">' + '<button class="btn-floating waves-effect waves-light green btn tooltipped addSwimResult" data-position="right" data-tooltip="Sauvegarder"' + 'value="add_' + data[_i2][_j]['ath_id'] + '" ><i id="add_' + data[_i2][_j]['ath_id'] + '" class="material-icons">add</i></button>' + '</div>' + '</div>' + '</td>' + '</form>' + '</tr>');
        _k2++;
        $('#modalSpace').append('<div id="foulsModal_' + data[_i2][_j]['id'] + '" class="modal modal-fixed-footer blue-grey darken-1 white-text">' + '<div class="modal-content container">' + '<h4 class="center">Fautes</h4>' + '<div class="card-content ">' + '<span class="card-title">' + data[_i2][_j]['last_name'] + ' ' + data[_i2][_j]['first_name'] + '</span>' + '<div class="row">' + '<div class="input-field col s12">' + '<select id="foulSelect_' + data[_i2][_j]['id'] + '">' + '<option value="" selected disabled>Choose your option</option>' + '</select>' + '<label>Sélectionnez la faute</label>' + '</div>' + '<div class="row">' + '<div>' + 'Pénalité appliquée : <span id="foulLabel_' + data[_i2][_j]['id'] + '"></span>' + '</div>' + '</div>' + '<div class="row">' + '<div>' + 'Points : <span id="foulPoints_' + data[_i2][_j]['id'] + '"></span>' + '</div>' + '</div>' + '<div class="row" >' + '<div class="collection" id="athFoulsList_' + data[_i2][_j]['id'] + '">' + '</div>' + '</div>' + '</div>' + '</div>' + '</div>' + '<div class="modal-footer blue-grey darken-1">' + '<div class="row">' + '<div class="col s4">' + '<button class="btn waves-effect waves-light orange forfeit" id="foulsValidate_' + data[_i2][_j]['id'] + '">Forfait' + '<i class="material-icons right">send</i>' + '</button>' + '</div>' + '<div class="col s4">' + '<button class="btn waves-effect waves-light red abandon" id="foulsValidate_' + data[_i2][_j]['id'] + '">Abandon / Disqual°' + '<i class="material-icons right">cancel</i>' + '</button>' + '</div>' + '<div class="col s4">' + '<button class="btn waves-effect waves-light green validate" id="foulsValidate_' + data[_i2][_j]['id'] + '">Valider' + '<i class="material-icons right">send</i>' + '</button>' + '</div>' + '</div>' + '</div>' + '</div>');
      }

      $('#' + gender + 'sHeatDistance' + _l + '').html('' + data[_i2][0]['lr_distance'] + 'm');
      _k2 = 1;
      _l++;
      $('select').formSelect();
      $('.modal').modal();
    ...
    for (var _i3 = 0; _i3 < Object.keys(data).length; _i3++) {
      $('#' + gender + 'sRes').append('<li>' + '<div class="collapsible-header heat" id="heat_' + _l2 + '_' + gender + 's"><i class="material-icons"><img src="https://img.icons8.com/ios-glyphs/24/000000/male.png"/></i>' + data[_i3][0]['cat_name'] + ' <span id="' + gender + 'sHeatDistance' + _l2 + '"> </span></div>' + '<div class="collapsible-body" id="heat_' + _l2 + '_' + gender + 's_content">' + '<table id="' + gender + 'sTable_' + _l2 + '">' + '<thead>' + '<tr>' + '<th class="center">Place</th>' + '<th class="center">Club</th>' + '<th class="center">Nom</th>' + '<th class="center">Natation</th>' + '<th class="center">Laser Run</th>' + '<th class="center">Total</th>' + '</tr>' + '</thead>' + '<tbody id="' + gender + 'sTable_' + _l2 + '_content">' + '</tbody>' + '</table>' + '</div>' + '</li>' + '</ul>');
